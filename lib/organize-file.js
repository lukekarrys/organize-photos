...
// Parse a photo booth filename to include the time from the file
const parsePhotoBooth = (file) => {
  const matched = file.match(/on (\d{1,2})-(\d{1,2})-(\d{1,2}) at (\d{1,2})\.(\d{1,2}) ([AP]M)/)
  if (matched) {
    let [, month, day, year, hour, minute, meridiem] = matched
...
module.exports = ({ ep, dest, ext, exifDate, real, command, constants }) => {
  ...
  return (file) => {
    ...
    const ogBase = path.basename(file.path, ogExt)
    ...
    return ep.readMetadata(ogPath)
      ...
      .then((item) => {
        console.log(Object.keys(item).filter((k) => k.includes('Date')).map((k) => `${k}  ${item[k]}`))
        let date = item[exifDate.find((prop) => item[prop])] || parsePhotoBooth(ogBase)
