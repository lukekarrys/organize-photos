...
module.exports = ({ ep, dest, ext, exifDate, real, command, verbose }) => {
  const copyDest = copyTo(dest, { real, command })
  const copyUnsorted = copyTo(path.join(dest, 'UNSORTED'), { real, command })
  const copyUnknown = copyTo(path.join(dest, 'UNKNOWN'), { real, command })
  ...
  return function (file) {
    ...
    return ep.readMetadata(ogPath)
      ...
      .then((item) => {
        const date = exifDate.find((prop) => item[prop])
      ...
      .then((date) => path.join(
        ...
        dateformat(date, 'yyyy-mm-dd HH-MM-ss') + path.extname(ogPath).toLowerCase()
      ))
