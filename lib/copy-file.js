...
const testCopy = (metaOptions = {}) => (src, dest, options) => {
  ...
  return new Promise((resolve, reject) => {
    if (fakeFs[dest]) {
      reject(new Error(`${dest} already exists`))
    } else {
      console.log(dest)
      fakeFs[dest] = true
      resolve()
...
// Only retries for `already exists` errors
const copyFile = (src, dest, options = {}, metaOptions = {}) => {
  ...
  return testCopy(metaOptions)(
    src,
    dest,
    Object.assign({ overwrite: false, errorOnExist: true, preserveTimestamps: true }, options)
  ).then((resp) => ({ src, dest })).catch((err) => {
    if (err && err.message.endsWith(' already exists')) {
