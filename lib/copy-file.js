...
const testCopy = (real, command) => (src, dest, options) => {
  if (real) return fs[command || 'copy'](src, dest, options)
...
const copyFile = (src, dest, options, real, command) => testCopy(real, command)(
    src,
    dest,
    ...
    if (err && err.message.startsWith('EEXIST')) {
      return copyFile(src, appendFileName(dest), options, real)
    }
    throw err
  })
